...
# - 入力と正解の識別用情報をtsvに変換
# - 正解にはlink先のタイトルをつける
# - htmlのテキスト、行番号、オフセットを使用
# usage
# - json2tsv_linkjp_html_simple.py gold_dir in_dir
# output
...
# notice
# - html(mention)部分\nを含む場合エスケープ(\\n)
# - get_key_plus:     tmp_list = [pid, at, html_text, start_line_id, start_offset, end_line_id, end_offset, link_id]
# - get_key_full:     tmp_list = [pid, at, html_text, start_line_id, start_offset, end_line_id, end_offset, link_id,
...
def get_key_full(**tr):
    ...
    if 'html_offset' not in tr:
        raise KeyError('(get_key_full)format_error: html_offset', tr)
        # sys.exit()
    else:
        if 'start' not in tr['html_offset']:
            raise KeyError('(get_key_full)format_error: start', tr)
        else:
            if 'line_id' not in tr['html_offset']['start']:
                raise KeyError('(get_key_full)format_error: line_id', tr)
            else:
                start_line_id = str(tr['html_offset']['start']['line_id'])

            if 'offset' not in tr['html_offset']['start']:
                raise KeyError('(get_key_full)format_error: offset', tr)
            else:
                start_offset = str(tr['html_offset']['start']['offset'])

        if 'end' not in tr['html_offset']:
            raise KeyError('(get_key_full)format_error: end', tr)
        else:
            if 'line_id' not in tr['html_offset']['end']:
                raise KeyError('(get_key_full)format_error: line_id', tr)
            else:
                end_line_id = str(tr['html_offset']['end']['line_id'])

            if 'offset' not in tr['html_offset']['end']:
                raise KeyError('(get_key_full)format_error: offset', tr)
            else:
                end_offset = str(tr['html_offset']['end']['offset'])

            if 'text' not in tr['html_offset']:
                raise KeyError('(get_key_full)format_error: text', tr)
            else:
                text = tr['html_offset']['text']
                # logger.debug({
                #     'action': 'get_key_full',
                #     'pid': pid,
                #     'html': html,
...
def get_key_plus(**tr):
    ...
    if 'html_offset' not in tr:
        raise KeyError('(get_key_plus)format_error: html_offset', tr)
        # sys.exit()
    else:
        if 'start' not in tr['html_offset']:
...
# def get_key(log_info, **tr):
    ...
    if 'html_offset' not in tr:
        # logger.error({
        #     'action': 'get_key',
        #     'error': 'format_error: html_offset',
        #     'tr': tr
        # })
        sys.exit()
    else:
        if 'start' not in tr['html_offset']:
            # logger.error({
            #     'action': 'get_key',
            #     'error': 'format_error: start(html_offset)',
            #     'tr': tr
            # })
            sys.exit()
        else:
        ...
        if 'end' not in tr['html_offset']:
            # logger.error({
            #     'action': 'get_key',
            #     'error': 'format_error: end(html_offset)',
            #     'tr': tr
            # })
            sys.exit()
        else:
            ...
            if 'text' not in tr['html_offset']:
                # logger.error({
                #     'action': 'get_key',
                #     'error': 'format_error: text(html_offset)',
                #     'tr': tr
                # })
                sys.exit()
            else:
...
# sys_tail = '/*.json'
logger.info({
    'action': 'json2tsv_linkjp_html_simple',
    'gold_dir': gold_dir,
...
for ename in glob(ene_files):
    eo_list = []
    fname_after = ename.replace('.json', '_html.tsv')
    ...
    with open(ename, mode='r', encoding='utf-8') as e, open(fname_after, 'w', encoding='utf-8') as eo:
        ...
        for eline in e:
            ...
            # "end":{"line_id":72,"offset":26},"text":"粉末"},"html_offset":{"start":{"line_id":72,"offset":21},
...
for gname in glob(gold_files):
    ...
    if not os.path.isfile(gname):
        ...
        # リンクタイプ情報も含む
        go_full_list = []
        gname_after = gname.replace('.json', '_html.tsv')
        g_cat_pre = gname.replace(gold_dir, '')
        g_cat = g_cat_pre.replace('.json', '')
        # print('g_cat', g_cat)
        logger.info({
            'action': 'json2tsv_linkjp_html_simple',
            'gname_after': gname_after,
            'gname': gname
        })
