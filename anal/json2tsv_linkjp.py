'''
   ...
   - システム出力ファイルをtsv化したファイル
     - リンクタイプ含む(該当すれば文字列の1）
   notice
   - text(mention)部分\nを含む場合エスケープ(\\n)
   - get_key_plus:     tmp_list = [pid, at, text, start_line_id, start_offset, end_line_id, end_offset, link_id]
   - get_key_full:     tmp_list = [pid, at, text, start_line_id, start_offset, end_line_id, end_offset, link_id, later_name, part_of, derivation_of]
...
for sys_list in json_sys_files:
    logger.info({
        'action': 'json2tsv_linkjp(sys_files)',
    })
    for sname in glob(sys_list):
        ...
        if not os.path.isfile(sname):
            ...
            so_list = []
            with open(sname, mode='r', encoding='utf-8') as s:
                sname_after = sname.replace('.json', '.tsv')
                s_cat_pre = sname.replace(sys_dir, '')
                s_cat = s_cat_pre.replace('.json', '')

                # print('s_cat', s_cat)
                with open(sname_after, 'w', encoding='utf-8') as so:
                    for sline in s:
                        # s_key_list = []
                        d_sline = json.loads(sline)

                        # s_key = sc.get_key_plus(log_info, **d_sline)
                        s_key = get_key_plus(**d_sline)
                        # s_key = get_key_plus(log_info, **d_sline)

                        # ['192643', 'タナ湖', '種類', '淡水湖', '35', '14', '35', '17', '2590', '']
                        # ['192643', '種類', '淡水湖', '35', '14', '35', '17', '2590', '']

                        # linkidまでを返す

                        s_key_list = re.split('\t', s_key)
                        ...
                        #     s_attr = s_key_list[1]
                        #
                        #     if check_del.get(s_cat):
                        #         if s_attr in check_del[s_cat]:
                        #             #print('s_del_attr', s_cat, s_attr)
                        #             continue
                        #         else:
                        #             if s_attr == '別名':
                        #                 print('別名', s_cat, s_attr)
                        #     # print(s_cat, s_attr)

                        # PID, title, redirectのファイルではタイトルが'_'で穴埋めされている
                        # titleとlink pageのtitleの表記のミスマッチを防ぐためPIDから取得
                        tmp_pid = s_key_list[0]
                        tmp_link_pid = s_key_list[7]
                        ...
                        s_key_list.insert(1, title)
                        s_key_list.append(link_title)
                        # textが複数行の場合、改行コード'\n'を'\\n'に変換する
                        text_pre = s_key_list[3]
                        s_key_list[3] = '\\n'.join(text_pre.splitlines())
                        so_list.append(s_key_list)
                    df_so = pd.DataFrame(so_list)
                    # print(df_fo)
                    df_so.to_csv(so, sep='\t', header=False, index=False)
